...
1. 클라이언트에서 수집한 데이터는 Edge Server로 전송됩니다. 또한 서버 측 수집 방법으로 먼저 데이터를 자체 서버로 보낼 수도 있습니다.
WebSDK가 서버 간 수집 기능을 제공할 수 있습니다. 그러나 이를 구현하려면 다른 프로그래밍 모델이 필요합니다. 아래의 **Edge Network Server API 개요** 설명서를 참조하세요.

1. Platform Edge Server는 데이터 수집 페이로드를 수신하여 Target, Analytics 등 필요한 시스템으로 데이터 흐름을 오케스트레이션합니다.
