...
위치 및 사용 패턴에 따라 개인화된 오퍼 및 혜택으로 업그레이드할 수 있는 고객을 대상으로 합니다. 새로운 적용 범위가 확장됨에 따라 관련 메시징을 통해 새롭게 적용되는 지역의 가입자에게 도달하면 채택이 가속화되고 네트워크 활용도가 증가합니다.

### 비즈니스에 대한 영향

타깃팅된 업그레이드 캠페인은 적격 가입자의 채택률을 향상시켜 네트워크 투자 수익을 지원하고 경쟁 우위를 확보합니다.

### 구현 방법

[일괄 아웃바운드 메시지 활성화](/help/blueprints/use-case-patterns/campaign-management-orchestration/batch-outbound-message-activation.md) 패턴을 사용하여 적용 범위 가용성, 장치 호환성 및 플랜 적격성에 따라 구독자를 세그먼트화한 다음 각 구독자의 사용 프로필과 가장 관련이 있는 이점을 강조 표시하는 개인화된 업그레이드 캠페인을 제공합니다. 이는 대상자가 사전 정의되어 있고 크기가 클 때, 이벤트 중심의 대신 전달 시기가 예약되어 있고, 실시간 분기 또는 의사 결정이 필요하지 않은 경우, 커버리지 롤아웃 타임라인을 기반으로 캠페인을 미리 완전히 계획할 수 있는 올바른 패턴입니다.

### 기술 고려 사항

- 네트워크 적용 범위 맵을 통합하여 활성 서비스가 있는 지역의 가입자를 정확하게 식별하고 아직 적용 범위를 사용할 수 없는 업그레이드를 촉진하지 않도록 합니다.
- 장치 호환성 데이터를 연결하여 이미 사용 가능한 하드웨어를 보유한 구독자와 비교하여 새 장치가 필요한 구독자를 파악합니다.
