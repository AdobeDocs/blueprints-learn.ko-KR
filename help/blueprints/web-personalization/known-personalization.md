...
* Adobe Analytics 또는 Customer Journey Analytics(선택 사항): 세그멘테이션을 통해 이전 고객 및 행동 데이터를 기반으로 세그먼트를 작성하는 기능을 추가합니다
...
* 소비자가 웹 사이트를 방문하거나 로그인하기 전에 허브에 업로드된 데이터는 즉시 개인화에 사용할 수 없습니다. 허브 데이터를에 동기화하려면 먼저 활성 에지 프로필이 있어야 합니다. 만들어진 Edge Profile은 비동기적으로 Hub 프로필과 동기화되어 다음 페이지 개인화가 발생합니다.
* Adobe Experience Platform에서 Adobe Target으로 대상을 공유하려면 위의 통합 파트너 2 및 3에 설명된 대로 대상 공유 서비스를 사용할 때 ECID를 ID로 사용해야 합니다.
* Audience Manager을 통해 대체 ID를 사용하여 Experience Platform 대상을 Adobe Target에 공유할 수도 있습니다. Experience Platform에서 Audience Manager를 활성화 가능한 지원되는 네임스페이스는 다음과 같습니다: IDFA, GAID, AdCloud, Google, ECID, EMAIL_LC_SHA256. Audience Manager 및 Target은 ECID ID를 통해 대상 멤버십을 해결하므로 Adobe Target에 대한 최종 대상을 공유하려면 소비자에 대한 ID 그래프에 ECID가 있어야 합니다.
