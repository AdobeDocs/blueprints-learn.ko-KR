...
XDM 개별 프로필 클래스 기반 스키마를 사용하면 프로필에 결합된 개인에 대한 속성을 수집할 수 있습니다. 클래스 자체에는 *modifiedByBatchID*, *PersonID* 등과 같이 편집할 수 없는 필드가 포함되어 있습니다.
...
Adobe Experience Platform에는 스키마에 추가하고 사용자 지정할 수 있는 표준 XDM으로 존재하는 필드 그룹이 많습니다.
...
1. **인구 통계 세부 정보**&#x200B;를 검색하거나 목록을 검색하여 찾으십시오.
   ...
   - 필드 그룹을 찾으면 필드 그룹의 오른쪽에 있는 돋보기를 클릭하여 해당 구조를 확인합니다.  이 단계는 추가하려는 항목을 스키마에 추가하지 않고 미리 보는 유용한 방법입니다.
   - 검토를 마치면 미리보기를 닫습니다.
...
완료되면 스키마가 아래 이미지와 유사합니다. **저장** 단추를 클릭하고 작업을 저장하십시오!
...
Adobe 표준 필드 그룹에서 필드를 제거하려면 **관련 필드 관리** 옵션을 사용하십시오. 관련 필드 관리를 사용하면 스키마에서 표준 필드를 제거할 수 있으므로 필요한 필드만 남습니다.
...
1. 완료되면 아래와 같이 스키마에 개인 오브젝트가 표시됩니다. 스키마를 저장하려면 **저장** 단추를 클릭하세요.
