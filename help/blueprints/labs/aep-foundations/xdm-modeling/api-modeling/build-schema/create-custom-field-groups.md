...
1. 다음 형식을 사용하여 필드 그룹의 `title` 및 `description`을(를) 업데이트합니다. `Customer Account Details - Sandbox <your number here>`
...
1. `Send` 단추를 클릭하여 실행하십시오.  아래 스크린샷과 유사한 응답이 표시됩니다.

1. 새로 만든 고객 계정 세부 정보 필드 그룹의 `$id` 값을 복사합니다.
