...
1. `Postman API Collection File`의 URL을 [가져오기] 모달 텍스트 상자에 붙여 넣습니다. 자동 가져오기를 트리거합니다.
...
가져온 API 컬렉션에는 부트캠프 전반에 걸쳐 Labs에 필요한 모든 API 호출이 포함되어 있습니다.  각 랩은 자체 API 세트와 함께 특정 폴더로 구성됩니다.  이번 주에 실습을 마칠 때 이 폴더 구조에 대해 알아두십시오.
