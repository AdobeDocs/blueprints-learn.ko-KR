...
1. `Environment File`의 URL을 오버레이의 가져오기 양식 텍스트 상자에 붙여 넣습니다.  자동 가져오기가 트리거됩니다.
...
가져온 후에는 왼쪽 사이드바의 `Environments` 탭을 클릭하여 환경 파일이 있는지 확인할 수 있습니다.  아래와 비슷한 게 보일 거예요.
...
API를 호출하려면 먼저 방금 가져온 환경 파일에서 몇 가지 변수를 업데이트해야 합니다.  이러한 변수는 API 호출에서 참조되므로 올바르게 입력되었는지 확인하십시오.  변수는 두 개의 그룹으로 나뉩니다.
...
1. 아래에 나열된 변수에 대해 `current values`을(를) 업데이트합니다.
   - CLIENT\_SECRET
...
>[!NOTE]
>
>샌드박스-assignment.pdf가 있는 라이브 교육 이벤트가 아닌 원하는 속도로 이러한 랩을 진행하는 경우 Adobe Experience Platform UI URL에서 샌드박스로 로그인된 상태에서 두 값을 모두 찾을 수 있습니다. 예를 들면 다음과 같습니다.
>
