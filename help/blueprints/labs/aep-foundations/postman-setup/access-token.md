...
Adobe 제품에 대한 보안 API 연결을 설정하기 위해 Adobe에서는 OAuth 서버 간 자격 증명 생성을 제공합니다. 이렇게 하려면 먼저 Adobe Developer Console 내에서 개발자 프로젝트를 만들어야 합니다. Developer Console에 액세스하려면 Adobe Admin Console 내의 개발자 권한을 할당받아야 합니다. 이러한 권한이 있으면 다양한 Adobe 제품 관련 API를 활용하는 개발자 프로젝트를 만듭니다. 이 시점에서 OAuth 서버 간 자격 증명을 사용합니다. 액세스 토큰을 생성하려면 Adobe의 Identity Management 서비스(IMS)에 특정 클레임 세트를 전달해야 합니다. OAuth 서버 간 자격 증명의 경우 예제 호출은 다음과 같습니다.
...
>[!NOTE]
>
>OAuth 서버 간 자격 증명 [여기](https://developer.adobe.com/developer-console/docs/guides/authentication/ServerToServerAuthentication/implementation/#generate-access-tokens)를 사용하여 개발자 프로젝트를 만드는 e2e 프로세스에 대해 자세히 알아보세요. 부트캠프의 경우 이 단계가 의도적으로 단순화되었습니다.



## Adobe Experience Platform + Adobe IMS

모든 Adobe 서비스에 대한 모든 요청에는 개발자 프로젝트 생성 중에 생성된 클라이언트 암호와 함께 인증 헤더에 액세스 토큰이 포함되어야 합니다. 또한 Experience Platform 및 관련 애플리케이션에는 각 요청에 대해 두 개의 다른 헤더 매개 변수가 필요합니다.
...
>[!CAUTION]
>
>`x-sandbox-name` 매개 변수를 지정하지 않으면 요청이 실패하지 않습니다. 대신, 처리할 요청의 기본값이 Experience Platform 환경에서 자동으로 프로비저닝되는 `default` 샌드박스로 설정됩니다

>[!NOTE]
>
>이 부트캠프에는 `access_token`을(를) 요청하는 데 필요한 모든 값이 포함된 개발자 프로젝트 및 Postman 환경 파일이 포함되어 있습니다. 이 환경 파일은 실습의 이전 단계에서 업로드한 것입니다
...
`token_type` - 은(는) 항상 형식 전달자입니다.

`access_token` - 권한 부여를 증명하며 모든 API 호출의 권한 부여 헤더에 필요합니다.

액세스 토큰이 만료될 때까지 `expires_in` - 밀리초(오늘 24시간 만료 기간)
...
이 오류는 환경 파일의 `private_key`이(가) 잘못되었거나 더 이상 유효하지 않을 때 발생합니다. 이 오류가 표시되면 줄 바꿈을 포함하여 전체 키를 복사했는지 확인합니다
