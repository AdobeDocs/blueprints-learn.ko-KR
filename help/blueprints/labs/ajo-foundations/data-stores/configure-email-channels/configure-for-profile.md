...
다음 단계에서는 `personalEmail.address` AEP 프로필 특성을 사용하여 여정 및 오케스트레이션된 캠페인을 모두 사용하는 이메일 채널 구성을 만듭니다
...
>[!NOTE]
>
>자습되고 사전 제공된 하위 도메인이 없는 경우 `email.dep-labs.com` 대신 여기에서 Adobe에 위임하는 고유한 하위 도메인을 선택하십시오. 하나를 위임하는 방법은 [설치](../../setup.md)를 참조하세요.
...
>[!NOTE]
>
>보낸 이메일 사본을 BCC 받은 편지함으로 보내 보관할 수 있습니다. 이 BCC 주소로 보낸 모든 이메일을 복사하려면 선택한 이메일 주소를 입력하십시오. BCC 주소 도메인은 Adobe에 위임된 하위 도메인과 달라야 합니다. 이 기능은 선택 사항입니다. *이메일에 숨은 참조를 사용하는 방법*
...
>[!NOTE]
>
>오케스트레이션된 캠페인의 경우 이메일로 고객 계정을 타겟팅하므로 프로필당 *한 개의 메시지*&#x200B;만 보내면 됩니다.  사용하는 실행 주소는 프로필 자체(특히 **personalEmail.address** 특성 아래의 AEP 프로필에 저장된 항목)에서 가져옵니다.
...
>[!NOTE]
>
>이메일 채널 구성 처리는 최대 2시간이 소요되는 것으로 관찰되었습니다!
>
