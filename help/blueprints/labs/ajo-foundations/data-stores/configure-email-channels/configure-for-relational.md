...
1. 목록 구독 취소에 대한 토글이 **활성화**&#x200B;인지 확인하십시오.
...
>[!NOTE]
>
>보낸 이메일의 사본을 보관하려면 BCC 받은 편지함으로 보냅니다. 전송된 모든 이메일이 이 BCC 주소로 이동하도록 선택한 이메일 주소를 입력합니다. BCC 주소 도메인은 Adobe에 위임된 하위 도메인과 달라야 합니다. 이 기능은 선택 사항입니다. *이메일에 숨은 참조를 사용하는 방법*
...
>[!NOTE]
>
>오케스트레이션된 캠페인의 경우 이메일로 고객 계정을 타겟팅하므로 Target Dimension당 하나의 메시지만 전송하면 됩니다.  사용하는 실행 주소는 Target Dimension 자체(즉, **전자 메일** 주소의 **dep-rel: Customer Account** 표에 저장된 주소)에서 가져옵니다.
