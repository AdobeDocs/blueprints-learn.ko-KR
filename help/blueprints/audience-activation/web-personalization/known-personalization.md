...
* Adobe Target 또는 Adobe Journey Optimizer Decisioning을 사용하여 웹 사이트 및 모바일 앱에서 실시간 고객 데이터 플랫폼에 정의된 대상자를 공유하고 타깃팅합니다.
...
* [Adobe Journey Optimizer 의사 결정](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/decisioning/gs-decision)
...
웹/모바일에서의 알려진 고객 개인화에 대한 한 가지 접근 방식은 실시간 고객 데이터 플랫폼을 Adobe Target과 통합하는 것입니다. 아래 안내서에서는 Real-time Customer Data Platform을 Adobe Target에 통합하여 알려진 고객 개인화를 향상시키는 방법에 대해 자세히 설명합니다.

알려진 고객 웹/모바일 개인화는 Adobe Journey Optimizer Real-time Customer Profile 및 Segmentation을 기본적으로 활용하는 Experience Platform Decisioning을 통해 구현할 수도 있습니다. [Adobe Journey Optimizer 가이드](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/ajo-home)를 참조하세요. 여기서 [코드 기반 경험](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/channels/code-based-experience/get-started-code-based) 또는 [웹 채널 경험](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/channels/web/get-started-web)을 사용할 수 있습니다.

아키텍처 블루프린트는 및 [Journey Optimizer 블루프린트](https://experienceleague.adobe.com/en/docs/blueprints-learn/architecture/customer-journeys/journey-optimizer/journey-optimizer) 및 [Journey Optimizer 의사 결정 블루프린트](https://experienceleague.adobe.com/en/docs/blueprints-learn/architecture/customer-journeys/journey-optimizer/decision-management/decision-management-overview)에서도 사용할 수 있습니다.
...
| **Edge 접근 방식을 통해 Real-time Customer Data Platform에서 Target으로 스트리밍 및 일괄 대상자 공유** | - Edge 네트워크를 통해 Real-time Customer Data Platform에서 Target으로 스트리밍 및 일괄 처리 대상자를 공유합니다. <br>- 실시간으로 평가되는 대상에는 웹 SDK 및 Edge Network 구현이 필요합니다. | - Target에 스트리밍 및 일괄 RTCDP 대상을 공유하는 데 Target의 웹/모바일 SDK 또는 Edge API 구현이 필요하지 않지만 실시간 에지 세그먼트 평가를 활성화하는 데 필요합니다. <br>- AT.js를 사용하는 경우 ECID ID 네임스페이스에 대한 프로필 통합만 지원합니다. <br>- Edge에서 사용자 지정 ID 네임스페이스 조회를 수행하려면 웹 SDK/Edge API를 배포해야 하며 각 ID는 ID 맵에서 ID로 설정해야 합니다. <br>- Real-time Customer Data Platform 대상에 대상 을 구성해야 합니다. RTCDP의 기본 프로덕션 샌드박스만 지원됩니다. <br>- Target과 통합하려면 Experience Platform 인스턴스와 동일한 IMS 조직이어야 합니다. |
...
* 이전에 RTCDP에 수집된 알려진 고객 데이터를 사용하여 첫 번째 로그인 개인화를 수행하려면 개인화 요청에 실시간 고객 데이터 플랫폼의 알려진 고객 ID 그래프와 일치하는 기본 ID가 있어야 합니다. 기본 ID가 ECID 또는 알려진 고객 프로필과 아직 결합되지 않은 ID로 설정된 경우 ID 결합이 에지에서 실현되고 에지 개인화가 이전에 수집된 알려진 고객 데이터를 포함하는 데 몇 분이 소요됩니다.
