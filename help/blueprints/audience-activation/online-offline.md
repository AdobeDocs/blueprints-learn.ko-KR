...
* 프로필로 수집할 Analytics 데이터가 활성화되어 있을 때 또는 Web SDK에서 직접 프로필에 대해 활성화해야 하는 별도의 프로필 레코드 클래스 데이터 세트로 Audience Manager Edge가 저장되는 지역 라우팅 정보를 Analytics 데이터 커넥터에서 수집할 수 있습니다.
