...
| 대상 정의 및 세분화 | 해당 사항 없음 | 이벤트 전달은 개별 이벤트를 실시간으로 처리하며 대상 멤버십을 평가하지 않습니다. 대상 기반 필터링은 이벤트 전달 실행 계획의 일부가 아닙니다. 대상자 기반 활성화가 필요한 경우 대상 Audience Activation 참조 계획을 참조하십시오. | |
...
**응용 프로그램 기능:** AEP: 이벤트 규칙 정의, AEP: 대상 매핑
