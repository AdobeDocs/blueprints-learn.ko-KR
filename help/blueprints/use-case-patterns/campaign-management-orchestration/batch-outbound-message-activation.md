...
- AJO 샌드박스가 프로비저닝되고 활성화됩니다.
- 전송 하위 도메인이 위임되고 확인됨(SPF, DKIM, DMARC 구성됨)
- IP 풀이 할당되고 프로덕션 전송 볼륨에 대해 워밍업됨
- 활성 채널 표면(이메일, SMS 또는 푸시)이 하나 이상 있음
- 사용자 계정에는 캠페인/여정 생성 및 컨텐츠 작성 권한이 있습니다.
- XDM 프로필 스키마에는 대상자 세분화 및 메시지 개인화에 필요한 속성이 포함됩니다
- XDM 이벤트 스키마는 캠페인 후 추적을 위해 전환 이벤트를 캡처합니다.
- 프로필 데이터는 ID 서비스를 통해 수집되고 통합됩니다
- 웹 SDK 또는 Analytics 태깅은 전환 이벤트를 캡처하기 위해 CTA 대상 페이지에서 활성화됩니다
- 동의 필드는 대상 메시징 채널의 프로필에 채워집니다
- 컨텐츠 자산(이미지, 로고, 브랜드 지침)을 메시지 디자인에 사용할 수 있습니다
