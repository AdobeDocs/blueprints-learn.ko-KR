...
* 메시지에만 기본적으로 Offer Decisioning 지원(사용자 정의 작업 불가)
* 지원 메시지 유형:
   * 이메일
   * 푸시(FCM/APNS)
