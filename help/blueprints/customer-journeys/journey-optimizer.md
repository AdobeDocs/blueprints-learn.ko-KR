...
* 기본적으로 메시지에서만 의사 결정 관리를 지원합니다(사용자 지정 작업 없음)
* 지원 메시지 유형:
   * 이메일
   * 푸시(FCM/APNS)
