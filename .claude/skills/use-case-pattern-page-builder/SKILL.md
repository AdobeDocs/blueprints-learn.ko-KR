...
5. **실행 계획 단계** — `>`(으)로 구분된 패턴의 실행 흐름을 설명하는 3~6개의 순차적 단계입니다. 예: &quot;이벤트 수집 > 여정 항목 > 조건 평가 > 메시지 게재 > 보고&quot;.
...
- 기본 기능 테이블(기능, 상태, 배치되어야 하는 사항, Experience League 참조)
- 지원 기능 표(기능, 상태, 중요 이유, Experience League 참조)
- 애플리케이션 기능 테이블(애플리케이션당 하나, 기능, 구현 단계, 설명 포함)
- 사전 요구 사항 체크리스트
...
7. **사용 사례 패턴** — 설명 단락 및 실행 계획입니다.

8. **응용 프로그램** — 서식 및 설명이 `[!DNL ...]`인 Adobe 응용 프로그램 목록입니다.

9. **기본 기능** — 기본 기능, 상태, 배치되어야 하는 항목, Experience League 참조 열이 있는 테이블. 상태 값: 필수, 적절히 가정, 해당 사항 없음.

10. **지원 기능** — 지원 기능, 상태, 중요한 이유, Experience League 참조 열이 있는 표 상태 값: 권장, 포함, 해당되지 않음.

11. **응용 프로그램 기능** - 응용 프로그램당 기능, 구현 단계, 설명 열이 있는 테이블 하나.
