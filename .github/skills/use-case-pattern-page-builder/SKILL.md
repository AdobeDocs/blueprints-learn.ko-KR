...
5. **지원되는 비즈니스 목표** — `/help/blueprints/business-objectives/`에 있는 기존 집합에서 하나 이상의 비즈니스 목표. 각 속성에는 목표 이름, 범주 하위 폴더 및 파일 이름이 포함되어야 합니다. 콘텐츠를 생성하기 전에 참조된 파일이 있는지 확인하십시오.

6. **전술적 사용 사례 예** — 이 패턴을 다양한 비즈니스 컨텍스트에 적용하는 방법을 설명하는 글머리 기호 시나리오가 6~10개 있습니다. 각 시나리오에는 굵은 시나리오 이름 뒤에 설명이 있어야 합니다.

7. **KPI** — KPI(이름), 설명(측정 항목), 측정(공식 또는 접근 방식)의 세 가지 열이 있는 테이블입니다.

8. **참조 링크** - 사용 사례 패턴의 응용 프로그램 및 기능에 대한 기본 Experience League 문서에 대한 참조 링크입니다.
...
사용자가 선택적 항목을 제공하지 않는 경우 패턴 범주, 솔루션 및 실행 계획에 따라 적절한 기본값을 생성합니다.
...
3. **사용 사례 패턴** — 설명 단락 및 실행 계획입니다.

4. **사용 사례 개요** — 패턴 범위, 적용 시, 수행할 작업과 수행할 수 없는 작업, 일반적인 이해 당사자를 설명하는 3~5개 단락입니다.

5. **주요 비즈니스 목표** - 각 목표는 간단한 설명과 KPI 요약 행이 있는 연결된 머리글입니다.

6. **전략 사용 사례 예** — 글머리 기호 6-10개 시나리오 목록입니다.

7. **주요 성능 지표** - KPI, 설명, 측정 열이 있는 테이블.
...
1. **비즈니스 목표 링크** — 패턴 파일의 모든 비즈니스 목표 링크는 `/help/blueprints/business-objectives/`에 있는 기존 파일을 가리킵니다. 작업 영역 검색 또는 파일 읽기를 사용하여 각 대상 파일이 있는지 확인합니다.
