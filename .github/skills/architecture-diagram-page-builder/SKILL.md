...
**선형 인터뷰가 아닌 사용 가능한 질문 양식을 사용하십시오.** 한 번에 한 질문을 하는 대신 논리 일괄 처리 라운드에서 모든 필수 정보를 수집합니다. 이렇게 하면 사용자에게 경험이 빠르고 스캔할 수 있습니다.
...
### 1라운드 — 핵심 페이지 정보(1개의 질문 양식, 최대 4개의 질문)
...
### 2라운드 — 다이어그램 세부 정보(1개의 질문 양식, 최대 4개의 질문)
...
### 3라운드 — 사용 사례 패턴(스캔 후 질문 양식)

이 양식을 제시하기 전에 `/help/blueprints/use-case-patterns/`을(를) 검색하고 페이지 제목, 목적 및 솔루션을 기준으로 3~5개의 일치하는 패턴을 식별하십시오. 제안하기 전에 각 파일이 있는지 확인하십시오.
...
### 4라운드 — 데이터 흐름 및 Experience League 링크(1개 질문 양식)
...
**Experience League 링크:** 양식 뒤에 문서 제목, URL 및 한 줄 이유가 있는 4~6개의 추천 링크로 구성된 Markdown 표를 제시합니다. 모든 URL을 **확인되지 않음**(으)로 표시합니다. (a) 수락하거나, (b) 확인된 URL로 대체하거나, (c) 직접 추가하도록 사용자에게 요청합니다. 목록이 길면 최대 4개의 옵션이 있는 후속 질문 양식을 사용하십시오. 그렇지 않으면 일반 텍스트로 확인을 수락하십시오.
...
2. **사용 사례 패턴 링크** — 파일의 모든 패턴 링크는 `/help/blueprints/use-case-patterns/` 아래의 기존 Markdown 파일을 가리킵니다. 작업 영역 검색 또는 파일 읽기를 사용하여 각 대상이 존재하는지 확인합니다.
...
- 설명 대체 텍스트, 상대 `assets/{filename}` 경로 및 다이어그램 확대/축소에 `{width="1000" zoomable="yes"}`을(를) 사용하는 Markdown 이미지를 사용합니다.
